...
var PluginRenamerSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    if (settingTabs.length > 0) {
      const heading = new import_obsidian.Setting(mainContentEl).setName("\u9009\u9879").setHeading();
    ...
    if (corePluginTabs.length > 0) {
      const heading = new import_obsidian.Setting(mainContentEl).setName("\u6838\u5FC3\u63D2\u4EF6").setHeading();
      const grid = createGridContainer(mainContentEl);
      groups.push({ headingEl: heading.settingEl, gridContainer: grid });
      corePluginTabs.forEach((tab) => renderSettingItem(grid, tab, false));
    }
    if (communityPluginTabs.length > 0) {
      const heading = new import_obsidian.Setting(mainContentEl).setName("\u7B2C\u4E09\u65B9\u63D2\u4EF6").setHeading();
      const grid = createGridContainer(mainContentEl);
      groups.push({ headingEl: heading.settingEl, gridContainer: grid });
